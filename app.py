...
pestaña = st.selectbox("Navegación:", ["🧑‍💻 Portal del Empleado", "📊 Panel de Gerencia", "🛠 Dueño del Software"], label_visibility="collapsed")
st.write("---")
...
# ==========================================
# 🛡️ SISTEMA ANTIFRAUDE (KILL SWITCH Y VENCIMIENTO)
# ==========================================
...
if pestaña == "🧑‍💻 Portal del Empleado":
    ...
    if not device_id:
        ...
        if empleado_en_celu:
                        ...
                        if not df_hoy_todos.empty:
                            ...
                            for e_comp in lista_empleados:
                                if e_comp != empleado_en_celu:
                                    ...
                                    if not df_c.empty:
                                        ult_c = df_c.iloc[-1]
                                        # Verifica que el compañero haya entrado y en la misma sucursal
                            ...
                            with st.form("form_sup_salida"):
                                ...
                                if st.form_submit_button("Fichar Salida"):
                                    if s_emp_salida == "Seleccionar...":
                                        ...
                                        if ya_salio:
                                            st.error(f"⚠️ El empleado {s_emp_salida} ya tiene una salida registrada en este turno. No podés enviarle otra.")
                                        elif ya_pedido:
                                            st.error(f"⚠️ Ya enviaste una solicitud de salida para {s_emp_salida} hoy. Gerencia la está revisando.")
                                        else:
                                            ...
                                            salidas_pendientes.append({
                                                ...
                                                "Sucursal": str(suc_cajero), "Turno": str(turno_del_auditado), "Nota": str(nota_final), "Autor": str(empleado_en_celu)
                                            })
            ...
            with st.expander("📍 Smart Check-In", expanded=True):
                ...
                if estado_laboral == "Fuera":
                    ...
                    if not config_app.get("exigir_salida_manual", False):
                        st.info("🤖 **Salida Automática Activada:** No necesitás registrar tu salida. El sistema computará tu turno automáticamente (descontando llegadas tarde). Si te retirás antes de tiempo, pedile al Responsable de Turno que audite tu salida.")
                    else:
                        puede_salir = True
                        ...
                        if config_app.get("salida_estricta", False) and local_actual in lista_locales:
                            st.markdown("🛡️ **Verificación requerida para finalizar turno:**")
            ...
            with st.expander("🕵️ Buzón de Reportes Confidenciales", expanded=False):
                ...
                if st.button("📩 Enviar a Gerencia"):
                    if not detalle_rep.strip(): st.warning("⚠️ Error: Tenés que escribir el detalle del reporte para poder enviarlo.")
    ...
    if password_ingresada == config_app.get("admin_password", "1234") or password_ingresada == "doremifasol":
        try:
            ...
            if 0 <= dias_restantes <= owner_config.get("dias_aviso", 5) and owner_config.get("estado_licencia", "Activo") == "Activo":
                st.markdown(f"<div class='task-pend' style='border-color: #F59E0B;'><b>🔔 Aviso del Proveedor de Software:</b><br>{owner_config.get('mensaje_aviso', '')} (Vence en {dias_restantes} días)</div>", unsafe_allow_html=True)
        except: pass
                    ...
                    for emp in df_dl["Empleado"].unique():
                        ...
                        for f in df_e["Fecha"].unique():
                            ...
                            if not ent.empty:
                                ...
                                # CÁLCULO DINÁMICO DE HORAS Y DESCUENTOS
        ...
        with tab_caja:
            ...
            if cierres_caja:
                ...
                else: st.info("ℹ️ No hay cierres de caja registrados en estas fechas.")
            else: st.info("ℹ️ Todavía no hay ningún cierre de caja registrado en el sistema.")
            ...
            with c_su1:
                with st.form("form_nuevo_sueldo"):
                    ...
                    if st.form_submit_button("💾 Guardar Tarifa"):
                        if emp_s == "Seleccionar..." or val_s <= 0: st.warning("⚠️ Tenés que seleccionar un empleado y poner un valor mayor a $0.")
                        elif f_ini > f_fin: st.warning("⚠️ La fecha 'Desde' no puede ser mayor a la fecha 'Hasta'.")
                        else:
        ...
        with tab_horarios:
            ...
            # --- MODIFICAR FICHAJES MANUALMENTE ---
            ...
            st.subheader("📅 Planilla Semanal de Turnos (Roster)")
            ...
            for emp in lista_empleados:
                ...
                for i, f_str in enumerate(str_fechas):
                    ...
                    if f_date > today_date:
                        ...
                        if plan_turno == "Libre":
                            cell_val = "✅ Libre" if real_estado == "No Fichó" else f"⚠️ Vino en su franco"
                        else:
                            if real_estado == "No Fichó": cell_val = f"⏳ Pendiente" if f_date == today_date else f"❌ Faltó sin aviso"
                                ...
                                if real_turno != plan_turno or real_sucursal != plan_loc: cell_val = f"⚠️ Vino a {real_sucursal} ({real_turno})"
                                else:
        ...
        with tab_puntos:
            ...
            with st.form("form_bonos"):
                ...
                if st.form_submit_button("Aplicar a Puntuación"):
                    if ap_emp == "Seleccionar..." or ap_puntos == 0 or not ap_motivo.strip(): st.warning("⚠️ Faltan completar datos.")
                    else:
        ...
        with tab_tareas:
            ...
            st.subheader("🕵️ Buzón de Quejas y Reportes")
                    ...
                    if c_mod1.button("💾 Guardar Cambios"):
                        ...
                        if nn and nn != emp_mod:
                            if nn not in lista_empleados:
                                ...
                                roles_empleados.pop(emp_mod, None)  # CORRECCIÓN DE BUG
        ...
        with tab_config:
            ...
            with col_aj2:
                ...
                with st.form("form_seguridad"):
                    ...
                    v_desc_tarde = st.checkbox("📉 Descontar Llegada Tarde del total de horas", value=config_app.get("desc_tarde", True))
                    v_desc_temp = st.checkbox("📉 Descontar Salida Temprano del total de horas", value=config_app.get("desc_temp", True))
    ...
    if pass_dueño == "SantiMaster2026":
        ...
        with tab_licencia:
            ...
            with st.form("form_licencia"):
                ...
                msg_aviso = st.text_area("Mensaje preventivo de vencimiento:", value=owner_config.get("mensaje_aviso", "⚠️ Tu suscripción está por vencer. Contactate con soporte para renovar."))
